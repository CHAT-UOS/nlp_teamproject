        ...
        for j in range(3, len(token_list)):
            ...
            if contains_english(new_data['label']):  # label단어가 영어인 경우
                ...
                for k in range(1, len(new_data["label"])):
                    last_token_list = token_list[j -
                                                 3: j] + [new_data["label"][:k]]
                    data_list.append(
                        {"data": last_token_list, "label": token_list[j]})
                    total_num += 1
            else:  # label 단어가 한국어인 경우
                split_korean_original = new_data["label"]
                split_korean_original, result = exclude_special_characters(
                    split_korean_original)
                if result == False:
                    continue

                index_list, split_korean = get_special_character_index_and_removed_string(
                    split_korean_original)
                        ...
                        for i in range(len(index_list)):
                            if index_list[i] <= len(recovered):
                                if recovered[:index_list[i]] == split_korean_original[:index_list[i]]:
                                    recovered = recovered[:index_list[i]] + \
                                        split_korean_original[index_list[i]
                                                              ] + recovered[index_list[i]:]
                            last_token_list = token_list[j -
                                                         3: j] + [recovered]
                        new_token = {"data": last_token_list,
                                     "label": token_list[j]}
                        typos_text = typos_generator(
                            new_token["data"][-1], model_weight_parameter, base_keyboard)
                        new_token["data"][-1] = typos_text
                        data_list.append(
                            {"data": last_token_list, "label": token_list[j]})
                        # print({"data" : last_token_list, "label" : token_list[j]})
                    else:
                        last_token_list = token_list[j - 3: j] + \
                            [combine_word(recover_word(split_korean[:k]))]
                        new_token = {"data": last_token_list,
                                     "label": token_list[j]}
                        typos_text = typos_generator(
                            new_token["data"][-1], model_weight_parameter, base_keyboard)
